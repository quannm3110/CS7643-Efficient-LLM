...
import datasets
from datasets import Features, MetricInfo, Value
...
def acc_and_f1(preds, labels, average="binary"):
    ...
    f1 = float(f1_score(y_true=labels, y_pred=preds, average=average))
    return {
        "accuracy": acc,
        "f1": f1,
    }
...
def acc_and_f1_micro_macro(preds, labels):
    acc = simple_accuracy(preds, labels)
...
class Glue(Metric):
    def _info(self):
        if self.config_name not in [
            ...
            "hans",
            "stack-exchange",
            "stack-exchange-with-context",
        ]:
            raise KeyError(
                "You should supply a configuration name selected in "
                '["sst2", "mnli", "mnli_mismatched", "mnli_matched", '
                '"cola", "stsb", "mrpc", "qqp", "qnli", "rte", "wnli", "hans", "stack-exchange", "stack-exchange-with-context"]'
            )
        return MetricInfo(
    ...
    def _compute(self, predictions, references):
        ...
        elif self.config_name in ["sst2", "mnli", "mnli_mismatched", "mnli_matched", "qnli", "rte", "wnli", "hans", "stack-exchange", "stack-exchange-with-context"]:
